...
type Server struct {
	...
	GetProfileByIDH goagrpc.UnaryHandler
...
func New(e *profile.Endpoints, uh goagrpc.UnaryHandler) *Server {
	return &Server{
		...
		GetProfileByIDH: NewGetProfileByIDHandler(e.GetProfileByID, uh),
	}
}
...
// NewGetProfileByIDHandler creates a gRPC handler which serves the "profile"
// service "get_profile_by_id" endpoint.
func NewGetProfileByIDHandler(endpoint goa.Endpoint, h goagrpc.UnaryHandler) goagrpc.UnaryHandler {
	if h == nil {
		h = goagrpc.NewUnaryHandler(endpoint, DecodeGetProfileByIDRequest, EncodeGetProfileByIDResponse)
	}
	return h
}

// GetProfileByID implements the "GetProfileByID" method in
// profilepb.ProfileServer interface.
func (s *Server) GetProfileByID(ctx context.Context, message *profilepb.GetProfileByIDRequest) (*profilepb.GetProfileByIDResponse, error) {
	ctx = context.WithValue(ctx, goa.MethodKey, "get_profile_by_id")
	ctx = context.WithValue(ctx, goa.ServiceKey, "profile")
	resp, err := s.GetProfileByIDH.Handle(ctx, message)
	if err != nil {
		var en goa.GoaErrorNamer
		if errors.As(err, &en) {
			switch en.GoaErrorName() {
	...
	return resp.(*profilepb.GetProfileByIDResponse), nil
}
